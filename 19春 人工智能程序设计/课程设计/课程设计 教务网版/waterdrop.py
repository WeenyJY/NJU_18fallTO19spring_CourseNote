...
def get_nearby_pix_value(img_pix,x,y,j):
 """获取临近5个点像素数据"""
 if j == 1:
  return 0 if img_pix[x-1,y+1] == 0 else 1  #左下
 elif j ==2:
  return 0 if img_pix[x,y+1] == 0 else 1   #下
 elif j ==3:
  return 0 if img_pix[x+1,y+1] == 0 else 1  #右下
 elif j ==4:
  return 0 if img_pix[x+1,y] == 0 else 1  #右
 elif j ==5:
  return 0 if img_pix[x-1,y] == 0 else 1 #左
 else:
...
def get_end_route(img,start_x,height):
 ...
 while cur_p[1] < (height-1):
  ...
  if last_p[0] == next_x and last_p[1] == next_y:
   if next_x < cur_p[0]:   #下一个点不变情况下，若下一个点x小于当前的，则向右下滴落
    max_w = 5
    next_x = cur_p[0] + 1
    next_y = cur_p[1] + 1
   else:
